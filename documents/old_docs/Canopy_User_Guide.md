...
Canopy is a modern C++ Remote Procedure Call library designed for type-safe communication across different execution contexts including in-process calls, inter-process communication, remote machines, embedded devices, and secure enclaves (SGX). The architecture supports multiple programming languages through IDL-based code generation and standardized wire protocols.
...
Canopy is designed to run the **same code** in both blocking and coroutine modes with minimal changes. This is achieved through conditional macros that adapt to the build configuration:
...
Canopy supports various reference types for different parameter passing semantics:
...
Canopy uses a **zone-based architecture** where each service instance operates in its own zone. Zones communicate through **service proxies** created using three main APIs:
...
Canopy implements a sophisticated distributed reference counting system that manages object lifetimes across multiple zones. Understanding this system is crucial for debugging distributed object lifecycle issues.
...
Canopy supports two reference types with different lifetime semantics:
...
- **Implementation**: `/rpc/include/rpc/internal/remote_pointer.h` (lines 231-1140)

**Optimistic References** (`rpc::optimistic_ptr`):
- **Fully implemented** weak reference semantics for remote objects
- Do NOT keep remote stubs alive (weak semantics)
- Stub can be deleted even if `optimistic_count > 0`
- Calling through optimistic_ptr after object deletion may return `OBJECT_GONE`
- Uses sophisticated control block architecture for efficient reference counting
- Supports both local and remote proxies with unified interface
- Factory functions: `make_optimistic()`, `dynamic_pointer_cast()`
- Use for circular dependencies, caches, stateless services, or non-owning access
- **Implementation**: `/rpc/include/rpc/internal/remote_pointer.h` (lines 1878-2066)

**Control Block Architecture**:
Both `rpc::shared_ptr` and `rpc::optimistic_ptr` use control blocks that manage:
- Local shared/weak reference counts (for local `rpc::shared_ptr`/`rpc::weak_ptr` copies)
- Optimistic reference counts (for `rpc::optimistic_ptr` copies)
- Delegation to `object_proxy` for distributed reference management
- Efficient memory management with disposal when all references reach zero
...
### Service Event Callbacks for Object Lifecycle

Applications can register event handlers to receive notifications when remote objects are destroyed. This is particularly useful for:
- Cleaning up `optimistic_ptr` instances when objects are deleted
- Updating caches and invalidating entries
- Implementing custom distributed garbage collection logic
- Debugging object lifecycle issues

**Service Event Interface** (from `/rpc/include/rpc/internal/service.h` lines 45-50):
...
public:
    ...
    // Called when a remote object with optimistic references is destroyed
    virtual CORO_TASK(void) on_object_released(rpc::object object_id,
                                               rpc::destination_zone destination) = 0;
};
```

**Implementing a Service Event Handler**:

```cpp
class my_object_lifecycle_handler : public rpc::service_event {
public:
    CORO_TASK(void) on_object_released(rpc::object object_id,
                                       rpc::destination_zone destination) override {
        // Handle object destruction notification
        RPC_INFO("Object {} in zone {} was destroyed",
                 object_id.get_val(), destination.get_val());

        // Clean up local optimistic_ptr instances
        cleanup_optimistic_references(object_id, destination);

        // Update caches, invalidate entries, etc.
        invalidate_cache_entries(object_id);

        CO_RETURN;
    }

private:
    void cleanup_optimistic_references(rpc::object object_id,
                                       rpc::destination_zone destination) {
        // Application-specific cleanup logic
        // Mark optimistic pointers as invalid, remove from collections, etc.
    }

    void invalidate_cache_entries(rpc::object object_id) {
        // Remove cached data associated with destroyed object
    }
};

// Register handler with service
auto lifecycle_handler = std::make_shared<my_object_lifecycle_handler>();
service->register_service_event(lifecycle_handler);
```

**How Notifications Work**:

1. **Object Deletion**: When a remote object's `shared_count_` reaches 0, the service deletes the stub
2. **Notification Dispatch**: Service calls `object_released()` on all zones with optimistic references
3. **Handler Invocation**: Registered `service_event` handlers receive `on_object_released()` callback
4. **Cleanup**: Handlers can clean up local state, invalidate caches, etc.

**Key Points**:
- Notifications are **unidirectional** (fire-and-forget, no response expected)
- Only zones with **optimistic references** receive notifications
- Handlers are called **asynchronously** via coroutine or blocking mode
- Multiple handlers can be registered on the same service
- Handlers use **weak_ptr** internally, so service doesn't keep them alive

**Use Cases**:
- **Cache Management**: Invalidate cached proxy objects when remote objects are destroyed
- **Resource Cleanup**: Release resources associated with destroyed objects
- **Monitoring**: Track object lifecycle for debugging and telemetry
- **Distributed GC**: Implement application-specific garbage collection strategies

**Reference**: See `/rpc/src/service.cpp` lines 817-845 for the `service::object_released()` implementation and lines 1161-1174 for the `notify_object_gone_event()` dispatcher.
...
Canopy implements a flexible error code system designed for seamless integration with legacy applications. The system allows error codes to be offset to prevent conflicts with existing application error codes.

### Design Principles

**Legacy Application Integration**: Canopy error codes can be offset to coexist with existing error code systems without conflicts:

```cpp
// Configure Canopy error codes to avoid conflicts
...
**Canopy-Internal Usage**: RPC error codes are reserved for internal Canopy operations and should only be used by applications for **inspection purposes**, not as application error codes.

### Core Error Codes

Canopy defines specific error codes for different failure scenarios:
...
Canopy implements automatic protocol version negotiation that gracefully handles version mismatches:
...
**Encoding Format Definition** (from `/rpc/interfaces/rpc/rpc_types.idl`):
```cpp
enum class encoding : uint64_t {
    yas_binary = 1,             // High-performance binary (default)
    yas_compressed_binary = 2,  // Space-optimized compressed binary
    yas_json = 8,               // Human-readable JSON (universal fallback)
    protocol_buffers = 16,      // Protocol Buffers (defined, check implementation status)
    // Future: flat_buffers = 32, msgpack, custom formats
};
```

**Format Features:**
- **Content Detection**: Automatic format detection from wire data
- **Performance Optimization**: Binary preferred, JSON fallback for compatibility
- **Debugging Support**: JSON format for wire-level inspection
- **Custom Formats**: Extensible architecture for domain-specific serialization
- **Universal Fallback**: `yas_json` guaranteed to work across all Canopy implementations

### Format Negotiation and Automatic Fallback

Canopy implements **automatic format negotiation** where clients and servers can discover compatible serialization formats without manual configuration. When a stub encounters an unsupported encoding format, it automatically triggers fallback logic.

**Supported Encoding Formats:**
```cpp
enum class encoding : uint64_t {
    yas_binary = 1,            // High-performance binary format (default)
    yas_compressed_binary = 2, // Space-optimized compressed binary format
    yas_json = 8,              // Universal fallback format (guaranteed compatibility)
    protocol_buffers = 16,     // Protocol Buffers (enum defined, check implementation)
};
```

**Note**: While `protocol_buffers` is defined in the encoding enum, full serialization implementation may be in progress. The YAS formats (`yas_binary`, `yas_compressed_binary`, `yas_json`) are fully implemented and production-ready.
...
**Key Benefits:**
- **Zero Configuration**: Applications work across different Canopy versions automatically
...
This dual negotiation ensures maximum compatibility across heterogeneous deployments with mixed Canopy versions and encoding capabilities.

### Deployment Protection

During code generation, Canopy creates a `check_sums/` directory structure organized by interface status:
...
The `i_marshaller` interface is the core abstraction that enables Canopy to work across different transport mechanisms. All methods return `CORO_TASK` types to support both blocking and coroutine execution modes.

**Reference**: `/rpc/include/rpc/internal/marshaller.h` (lines 97-182)
...
public:
    ...
    // Bidirectional RPC call - waits for response
    ...
    // Unidirectional RPC call - no response expected (fire-and-forget)
    ...
    // Query if remote object implements specific interface
    ...
    // Increment reference count on remote object
    ...
    // Decrement reference count on remote object
    ...
    // Notify zones with optimistic references that object was destroyed
    ...
    // Notify that transport connection is down
    // Unidirectional notification
...
- **All methods use `CORO_TASK` types** - enables bi-modal execution (blocking/coroutine)
- **Back channel parameters** - used for routing through intermediate zones (pass-through pattern)
- **Bidirectional vs Unidirectional**:
  - `send()` - waits for response, returns result
  - `post()`, `object_released()`, `transport_down()` - fire-and-forget notifications
- **Reference Counting** - `add_ref()` and `release()` return reference counts via output parameter
...
Canopy provides different service proxy types optimized for specific transport scenarios. Understanding their characteristics is essential for choosing the correct proxy type.
...
- **Bi-modal execution**: Work in both synchronous (`CANOPY_BUILD_COROUTINE=OFF`) and coroutine (`CANOPY_BUILD_COROUTINE=ON`) modes
...
**Key Characteristics**:
- **Coroutine-only**: Requires `CANOPY_BUILD_COROUTINE=ON`
...
- ✅ Coroutines are available (`CANOPY_BUILD_COROUTINE=ON`)
...
# SGX configurations
cmake --preset SGX_Debug      # Debug build with SGX hardware support
cmake --preset SGX_Sim_Debug  # Debug build with SGX simulation

# Custom configuration
cmake --preset Debug -DBUILD_COROUTINE=ON -DCANOPY_USE_LOGGING=ON
```
...
```cmake
# Core features
CANOPY_BUILD_COROUTINE=ON             # Enable coroutine support (TCP, SPSC transports)
CANOPY_BUILD_ENCLAVE=ON               # Enable SGX enclave support
CANOPY_BUILD_TEST=ON                  # Build test suite

# Debugging and development
CANOPY_USE_LOGGING=ON             # Enable comprehensive logging
CANOPY_USE_THREAD_LOCAL_LOGGING=ON    # Enable thread-local diagnostic logging
CANOPY_DEBUG_GEN=ON               # Debug code generation
...
**When `CANOPY_BUILD_COROUTINE=OFF` (Blocking Mode):**
```cpp
...
**When `CANOPY_BUILD_COROUTINE=ON` (Coroutine Mode):**
```cpp
...
Canopy uses a structured logging system based on fmt::format:
...
**Logging Features:**
- **Zero-overhead when disabled** (`CANOPY_USE_LOGGING=OFF`)
...
Some transport types are only available when `CANOPY_BUILD_COROUTINE=ON`:
...
Canopy provides automatic format negotiation and fallback mechanisms to ensure compatibility across different encoding formats and protocol versions. This system enables seamless communication between components that may support different serialization formats or RPC protocol versions.

### Supported Encoding Formats

Canopy supports multiple encoding formats with an extensible architecture.

**Fully Implemented Formats**:
- **`rpc::encoding::yas_binary` (1)** - Binary YAS serialization (compact, fast, default)
- **`rpc::encoding::yas_compressed_binary` (2)** - Compressed binary YAS (smallest size)
- **`rpc::encoding::yas_json` (8)** - JSON-based YAS serialization (universal compatibility, guaranteed fallback)

**Defined in Enum** (from `/rpc/interfaces/rpc/rpc_types.idl`):
- **`rpc::encoding::protocol_buffers` (16)** - Enum value defined, serialization implementation may be in progress

**Extensible Architecture**: The encoding system is designed to support additional serialization formats such as:
- **MessagePack** - Can be added as new enum value
- **Apache Avro** - Can be added as new enum value
- **FlatBuffers** - Already commented in IDL as future format (32)
- **Custom Formats** - Application-specific serialization schemes

**Implementation Status**: The YAS formats are production-ready and fully tested. For `protocol_buffers` or other formats, check current implementation status in the codebase before use.
...
The Canopy code generator creates automatic fallback logic in both proxy and stub code:
...
Canopy also provides automatic version negotiation for protocol compatibility:
...
- **`yas_json` encoding** works with all Canopy implementations (universal compatibility)
...
For testing fallback mechanisms, Canopy provides direct proxy testing approaches:
...
1. **Seamless Interoperability**: Different Canopy versions can communicate automatically
...
The Canopy encoding system is designed for extensibility. To add new formats like MessagePack or Apache Avro:

1. **Extend the Encoding Enum**: Add new encoding types to `rpc::encoding` in `/rpc/interfaces/rpc/rpc_types.idl`
...
**Note**: `protocol_buffers` enum value is already defined (value 16) in `rpc_types.idl`. If implementing Protocol Buffers support, only steps 2-4 are needed.

**Example MessagePack Integration**:
```cpp
// 1. Add to encoding enum in rpc_types.idl
namespace rpc {
    enum class encoding : uint64_t {
        yas_binary = 1,
        yas_compressed_binary = 2,
        yas_json = 8,
        protocol_buffers = 16,  // Already defined
        msgpack = 32,           // New format to add
        custom_format = 64      // Application-specific
    };
}

// 2. Generated stub code automatically validates all defined formats
if (enc != rpc::encoding::yas_binary &&
    enc != rpc::encoding::yas_compressed_binary &&
    enc != rpc::encoding::yas_json &&
    enc != rpc::encoding::protocol_buffers &&
...
// 3. Fallback logic works automatically for any format
if(__rpc_ret == rpc::error::INCOMPATIBLE_SERIALISATION())
{
    if(__rpc_encoding != rpc::encoding::yas_json)
    {
...
The automatic fallback system ensures that Canopy applications maintain compatibility while allowing individual components to optimize for their specific requirements, whether using built-in formats or custom extensions like Protocol Buffers.
...
This implementation now supports both **synchronous calls** and **asynchronous coroutines** (C++20), with the mode controlled at build time by the `CANOPY_BUILD_COROUTINE` macro.
...
```
  option(CANOPY_BUILD_ENCLAVE "build enclave code" ON)
  option(CANOPY_BUILD "build host code" ON)
  option(CANOPY_BUILD_EXE "build exe code" ON)
  option(CANOPY_BUILD_TEST "build test code" ON)
  option(CANOPY_DEBUG_LEAK "enable leak sanitizer (only use when ptrace is accessible)" OFF)
  option(CANOPY_DEBUG_ADDRESS "enable address sanitizer" OFF)
  option(CANOPY_DEBUG_THREAD "enable thread sanitizer (cannot be used with leak sanitizer)" OFF)
  option(CANOPY_DEBUG_UNDEFINED "enable undefined behaviour sanitizer" OFF)
  option(CANOPY_DEBUG_ALL "enable all sanitizers" OFF)
  option(CANOPY_DEBUG_ENCLAVE_MEMLEAK "detect memory leaks in enclaves" OFF)
  option(CANOPY_ENABLE_CLANG_TIDY "Enable clang-tidy in build (needs to build with clang)" OFF)
  option(CANOPY_ENABLE_CLANG_TIDY_FIX "Turn on auto fix in clang tidy" OFF)
  option(CANOPY_ENABLE_COVERAGE "Turn on code coverage" OFF)
  ...
  option(CANOPY_USE_LOGGING "turn on rpc logging" OFF)
  option(CANOPY_HANG_ON_FAILED_ASSERT "hang on failed assert" OFF)
  option(CANOPY_USE_TELEMETRY "turn on rpc telemetry" OFF)
  option(CANOPY_USE_TELEMETRY_RAII_LOGGING "turn on the logging of the addref release and try cast activity of the services, proxies and stubs" OFF)
 ```

**Note**: For current build configurations, see `CMakePresets.json` which provides preconfigured builds including `Debug`, `Debug_multithreaded`, `SGX_Debug`, `Release`, etc.
...
Canopy provides comprehensive support for C++20 coroutines, allowing you to write asynchronous RPC code that looks and feels like synchronous code while providing the performance benefits of non-blocking I/O.

#### Build Configuration

Coroutine support is controlled by the `CANOPY_BUILD_COROUTINE` CMake option and can be enabled at build time:
...
When `CANOPY_BUILD_COROUTINE=ON`:
...
When `CANOPY_BUILD_COROUTINE=OFF`:
...
public:
    // Returns coro::task<int> when CANOPY_BUILD_COROUTINE=ON
    // Returns int when CANOPY_BUILD_COROUTINE=OFF
...
Canopy provides macros that adapt based on the build configuration:
...
- **Use Coroutines (`CANOPY_BUILD_COROUTINE=ON`) when**:
...
- **Use Synchronous (`CANOPY_BUILD_COROUTINE=OFF`) when**:
...
1. **Start Synchronous**: Begin development with `CANOPY_BUILD_COROUTINE=OFF`
...
*This guide covers Canopy version 2.2.0. For the latest updates and detailed API documentation, visit the project repository.*