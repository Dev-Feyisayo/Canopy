...
#include <rpc/rpc.h>
#include <dodgy/dodgy.h>
#include <transports/dodgy/queue.h>

namespace rpc::dodgy
{
    using message_blob = std::array<uint8_t, 10024>;
    using queue_type = ::dodgy::queue<message_blob, 10024>;

    class dodgy_transport : public rpc::transport
    {
    public:
        using connection_handler = std::function<CORO_TASK(int)(const rpc::interface_descriptor& input_descr,
            ...
            std::shared_ptr<dodgy_transport>)>;
    ...
    private:
        ...
        stdex::member_ptr<dodgy_transport> keep_alive_;
        ...
        dodgy_transport(std::string name,
    ...
    public:
        static std::shared_ptr<dodgy_transport> create(std::string name,
            std::shared_ptr<rpc::service> service,
            rpc::zone adjacent_zone_id,
            queue_type* send_spsc_queue,
            queue_type* receive_spsc_queue,
        ...
        virtual ~dodgy_transport() = default;
        ...
        // Dodgy transport specific: simulate a network failure
        // This will trigger transport_down events on both sides of the connection
        CORO_TASK(void) trigger_network_failure();
    };
}
