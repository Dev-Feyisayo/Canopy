...
namespace json_schema
{
    ...
    std::string map_basic_type_to_json(const std::string& idl_type)
    {
        ...
        // String types
                 ...
                 || clean_type == "std::u16string" || clean_type == "std::u32string" || clean_type == "std::string_view"
                 || clean_type == "std::wstring_view" || clean_type == "std::u16string_view"
                 || clean_type == "std::u32string_view")
        {
            return "string";
        }
        ...
        std::set<std::string>& visited_types)
    {
        ...
        if (!template_args.empty())
        {
                ...
                && template_args.size() >= 1)
            {
                ...
                writer.write_key("items");
                map_idl_type_to_json_schema_enhanced(root, current_context, template_args[0], {}, writer, visited_types);
                writer.close_object();
                return;
            }
            else if ((container_name == "std::map" || container_name == "std::unordered_map") && template_args.size() == 2)
            {
                ...
                writer.write_key("v");
                map_idl_type_to_json_schema_enhanced(root, current_context, template_args[1], {}, writer, visited_types);
