...
namespace rpc
{
        ...
        rpc::caller_zone caller_zone_id) const
    {
        for (const auto& child : children_)
        {
            if (child)
            {
                child->on_transport_outbound_transport_down(zone_id, adjacent_zone_id, destination_zone_id, caller_zone_id);
            }
        }
    }
        ...
        rpc::caller_zone caller_zone_id) const
    {
        for (const auto& child : children_)
        {
            if (child)
            {
                child->on_transport_inbound_transport_down(zone_id, adjacent_zone_id, destination_zone_id, caller_zone_id);
            }
        }
    }
}
