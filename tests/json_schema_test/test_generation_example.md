...
The IDL code generation is integrated into the build system using `CanopyGenerate()` function:
...
The `CanopyGenerate()` function in `/tests/idls/CMakeLists.txt` handles the code generation automatically.
