...
public:
    ...
    CORO_TASK(int) cleanup(rpc::shared_ptr<i_garbage_collector> collector) override
    {
        ...
        for (auto& pair : cache_storage_)
        {
            ...
            if (cleanup_obj)
            {
                CO_AWAIT cleanup_obj->cleanup(collector);
                CO_AWAIT collector->collect(cleanup_obj);
            }
        }
...
public:
    ...
    CORO_TASK(int) process_object(rpc::shared_ptr<i_shared_object> object, int increment) override
    {
        ...
        {
            auto set_result = CO_AWAIT object->set_value(current_value + increment);
...
private:
    ...
    CORO_TASK(void) create_local_factory()
    {
        ...
        auto child_transport = std::make_shared<rpc::local::child_transport>(factory_zone_name.c_str(),
            current_service->shared_from_this(),
            zone_id,
            rpc::local::parent_transport::bind<i_fuzz_factory, i_fuzz_factory>(zone_id,
    ...
    CORO_TASK(void) create_local_cache()
    {
        if (local_cache_)
            CO_RETURN;
        auto current_service = rpc::service::get_current_service();
        ...
        auto child_transport = std::make_shared<rpc::local::child_transport>(cache_zone_name.c_str(),
            current_service->shared_from_this(),
            rpc::zone{zone_id},
            rpc::local::parent_transport::bind<i_fuzz_cache, i_fuzz_cache>(zone_id,
    ...
    CORO_TASK(void) create_local_worker()
    {
        if (local_worker_)
            CO_RETURN;
        auto current_service = rpc::service::get_current_service();
        ...
        auto child_transport = std::make_shared<rpc::local::child_transport>(worker_zone_name.c_str(),
            current_service->shared_from_this(),
            rpc::zone{zone_id},
            rpc::local::parent_transport::bind<i_fuzz_worker, i_fuzz_worker>(zone_id,
    ...
    CORO_TASK(rpc::shared_ptr<i_shared_object>) create_shared_object_via_factory(int object_id)
    {
        if (!local_factory_)
        {
        ...
        if (CO_AWAIT local_factory_->create_shared_object(object_id, obj_name, object_id * 10, new_obj) == rpc::error::OK()
            && new_obj)
        {
    ...
    CORO_TASK(void) store_object_in_cache(int cache_key, rpc::shared_ptr<i_shared_object> object)
    {
        ...
        if (object)
        {
            CO_AWAIT local_cache_->store_object(cache_key, object);
        }
        CO_RETURN;
    }
    ...
    CORO_TASK(void) process_object_via_worker(int increment, rpc::shared_ptr<i_shared_object> object)
    {
        ...
        if (object)
        {
            CO_AWAIT local_worker_->process_object(object, increment);
        }
        CO_RETURN;
    }
...
public:
    CORO_TASK(rpc::shared_ptr<i_autonomous_node>) fork_child_node()
    {
        ...
        auto result = CO_AWAIT create_child_node(node_type::WORKER_NODE, child_zone_id, true, child_node);
    ...
    CORO_TASK(int) run_script(rpc::shared_ptr<i_autonomous_node> target_node, int instruction_count) override
    {
        ...
        for (const auto& instruction : instructions)
        {
            ...
            {
                CO_AWAIT target_node->receive_object(current_object, node_id_);
            }
            else
            {
                rpc::shared_ptr<i_shared_object> output_object;
                CO_AWAIT execute_instruction(instruction, current_object, output_object);
                if (output_object)
                {
        ...
        rpc::shared_ptr<i_shared_object>& output_object) override
    {
        ...
        try
        {
            if (instruction.operation == "CREATE_CAPABILITY")
            {
                ...
                {
                case 0:
                    CO_AWAIT create_local_factory();
                    break;
                case 1:
                    CO_AWAIT create_local_cache();
                    break;
                case 2:
                    CO_AWAIT create_local_worker();
                    break;
                }
            }
            else if (instruction.operation == "CREATE_OBJECT")
            {
                output_object = CO_AWAIT create_shared_object_via_factory(instruction.target_value);
            }
            else if (instruction.operation == "STORE_OBJECT")
            {
                CO_AWAIT store_object_in_cache(instruction.target_value, input_object);
            }
            else if (instruction.operation == "PROCESS_OBJECT")
            {
                CO_AWAIT process_object_via_worker(instruction.target_value, input_object);
            }
            else if (instruction.operation == "FORK_CHILD")
            {
                CO_AWAIT fork_child_node();
            }
            else if (instruction.operation == "PASS_TO_RANDOM_CHILD")
            {
                if (!child_nodes_.empty())
                {
                    ...
                    CO_AWAIT target_child->receive_object(input_object, node_id_);
                }
            }
            else if (instruction.operation == "PASS_TO_PARENT")
            {
                if (parent_node_)
                {
                    CO_AWAIT parent_node_->receive_object(input_object, node_id_);
                }
            }
            else if (instruction.operation == "PASS_TO_RANDOM_SIBLING")
            {
                ...
                {
                    int sibling_count = 0;
                    CO_AWAIT parent_node_->get_cached_children_count(sibling_count);
                    if (sibling_count > 1)
                    {
                        ...
                        {
                            int index = dist(gen);
                            CO_AWAIT parent_node_->get_cached_child_by_index(index, target_sibling);
                        ...
                        CO_AWAIT target_sibling->receive_object(input_object, node_id_);
                    }
                }
            }
            else
        ...
        node_type child_type, uint64_t child_zone_id, bool cache_locally, rpc::shared_ptr<i_autonomous_node>& child_node) override
    {
        ...
        try
        {
            ...
            // Create a new parent controller node (cannot use shared_from_this in stub context)
            rpc::shared_ptr<i_autonomous_node> parent_controller
                = rpc::make_shared<autonomous_node_impl>(node_type_, node_id_);

            auto child_transport = std::make_shared<rpc::local::child_transport>(child_zone_name.c_str(),
                current_service->shared_from_this(),
                zone_id,
                rpc::local::parent_transport::bind<i_autonomous_node, i_autonomous_node>(zone_id,
    ...
    CORO_TASK(int) cleanup(rpc::shared_ptr<i_garbage_collector> collector) override
    {
        ...
        for (size_t i = 0; i < child_nodes_.size(); ++i)
        {
            ...
            auto cleanup_child = CO_AWAIT rpc::dynamic_pointer_cast<i_cleanup>(child);
            if (cleanup_child)
            {
                CO_AWAIT cleanup_child->cleanup(collector);
                CO_AWAIT collector->collect(cleanup_child);
            }
            else
            {
        ...
        for (size_t i = 0; i < created_objects_.size(); ++i)
        {
            ...
            auto cleanup_obj = CO_AWAIT rpc::dynamic_pointer_cast<i_cleanup>(obj);
            if (cleanup_obj)
            {
                CO_AWAIT cleanup_obj->cleanup(collector);
                CO_AWAIT collector->collect(cleanup_obj);
        ...
        if (local_factory_)
        {
            ...
            auto cleanup_factory = CO_AWAIT rpc::dynamic_pointer_cast<i_cleanup>(local_factory_);
            if (cleanup_factory)
            {
                CO_AWAIT cleanup_factory->cleanup(collector);
                CO_AWAIT collector->collect(cleanup_factory);
            }
        }
        ...
        if (local_cache_)
        {
            ...
            auto cleanup_cache = CO_AWAIT rpc::dynamic_pointer_cast<i_cleanup>(local_cache_);
            if (cleanup_cache)
            {
                CO_AWAIT cleanup_cache->cleanup(collector);
                CO_AWAIT collector->collect(cleanup_cache);
            }
        }
        ...
        if (local_worker_)
        {
            ...
            auto cleanup_worker = CO_AWAIT rpc::dynamic_pointer_cast<i_cleanup>(local_worker_);
            if (cleanup_worker)
            {
                CO_AWAIT cleanup_worker->cleanup(collector);
                CO_AWAIT collector->collect(cleanup_worker);
            }
        }
...
public:
    ...
    CORO_TASK(void) debug_print_collected_objects()
    {
        ...
        for (auto obj : collected_objects_)
        {
            ...
            auto shared_obj = CO_AWAIT rpc::dynamic_pointer_cast<i_shared_object>(obj);
            if (shared_obj)
            {
                int stats = 0;
                int value = 0;
                if (CO_AWAIT shared_obj->get_stats(stats) == rpc::error::OK()
                    && CO_AWAIT shared_obj->get_value(value) == rpc::error::OK())
                {
            ...
            auto cache = CO_AWAIT rpc::dynamic_pointer_cast<i_fuzz_cache>(obj);
            if (cache)
            {
                int cache_size = 0;
                if (CO_AWAIT cache->get_cache_size(cache_size) == rpc::error::OK())
                {
    ...
    rpc::shared_ptr<i_autonomous_node> parent, int depth, std::vector<rpc::shared_ptr<i_autonomous_node>>& all_nodes)
{
    ...
    for (int i = 0; i < depth; ++i)
    {
        ...
        auto result = CO_AWAIT current_node->create_child_node(node_type::WORKER_NODE, child_zone_id, true, new_child);
...
#endif
{
    ...
    auto root_service = std::make_shared<rpc::service>("AUTONOMOUS_ROOT", rpc::zone{++g_zone_id_counter}, scheduler);
#else
    ...
    // IMPORTANT: Register stubs in root service BEFORE creating child zones
    ...
    try
    {
...

        {
            ...
            auto child_transport = std::make_shared<rpc::local::child_transport>(zone_name.c_str(),
                root_service->shared_from_this(),
                new_zone_id,
                rpc::local::parent_transport::bind<i_autonomous_node, i_autonomous_node>(new_zone_id,
            ...
            auto root_cleanup = CO_AWAIT rpc::dynamic_pointer_cast<i_cleanup>(root_node);
            if (root_cleanup)
            {
                CO_AWAIT root_cleanup->cleanup(garbage_collector);
                CO_AWAIT garbage_collector->collect(root_cleanup);
            }
            else
            {
            ...
            for (auto node : all_nodes)
            {
                ...
                if (node_cleanup)
                {
                    CO_AWAIT node_cleanup->cleanup(garbage_collector);
                    CO_AWAIT garbage_collector->collect(node_cleanup);
                }
            }

            // DEBUG: Print all objects in garbage collector
            CO_AWAIT garbage_collector->debug_print_collected_objects();

        } // End of inner scope - automatic C++ destructors run here

        // Get count of objects transferred to garbage collector
        int collected_count = 0;
        CO_AWAIT garbage_collector->get_collected_count(collected_count);
