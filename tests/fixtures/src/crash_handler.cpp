...
namespace crash_handler
{
    ...
    void crash_handler::print_crash_report(const crash_report& report)
    {
        ...
        for (size_t i = 0; i < report.all_threads.size(); ++i)
        {
            ...
            std::cout << "\nThread " << (i + 1) << "/" << report.all_threads.size() << " - PID: " << thread.thread_id;
            ...
            {
                std::cout << "  " << std::setw(2) << j << ": " << format_address(thread.stack_frames[j]) << " "
                          << frames[j] << std::endl;
            }
    ...
    std::vector<std::string> crash_handler::resolve_symbols(const std::vector<void*>& addresses)
    {
        ...
        for (size_t i = 0; i < addresses.size(); ++i)
        {
            ...
            else if (symbol_strings && symbol_strings[i])
            {
                ...
                if (start_paren != std::string::npos && plus_pos != std::string::npos && end_paren != std::string::npos
                    && start_paren < plus_pos && plus_pos < end_paren)
                {
    ...
    {
        std::stringstream ss;
        ss << "0x" << std::hex << std::setfill('0') << std::setw(16) << reinterpret_cast<uintptr_t>(address);
        return ss.str();
    }
    ...
    std::string crash_handler::format_stack_trace_for_file(const crash_report& report)
    {
        ...
        if (test_info != nullptr)
        {
            ss << "Current Test: " << test_info->test_suite_name() << "." << test_info->name() << "\n";
        }
        ...
        {
            const auto& thread = report.all_threads[i];
            ss << "Thread " << (i + 1) << "/" << report.all_threads.size() << " - PID: " << thread.thread_id;
            ...
            for (size_t j = 0; j < thread.stack_frames.size() && j < frames.size(); ++j)
            {
                ss << "  " << std::setw(2) << j << ": " << format_address(thread.stack_frames[j]) << " " << frames[j]
                   << "\n";
            }
    ...
    void crash_handler::save_crash_dump(const crash_report& report)
    {
        ...
        filename << config_.crash_dump_path << "/crash_" << getpid() << "_" << time_t << ".txt";
        ...
        if (file.is_open())
        {
            ...
            {
                const auto& thread = report.all_threads[i];
                file << "Thread " << (i + 1) << "/" << report.all_threads.size() << " - PID: " << thread.thread_id;
                ...
                for (size_t j = 0; j < thread.stack_frames.size() && j < frames.size(); ++j)
                {
                    file << "  " << std::setw(2) << j << ": " << format_address(thread.stack_frames[j]) << " "
                         << frames[j] << "\n";
                }
            ...
            file << "- Console output (if CANOPY_USE_CONSOLE_TELEMETRY=ON)\n";
            file << "- Telemetry service logs (if CANOPY_USE_TELEMETRY=ON)\n";
